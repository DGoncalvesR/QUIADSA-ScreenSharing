...
function initializePlyrPlayers(playLocalPreview = false) {
  ...
  try {
    ...
    setError("Error al inicializar/reiniciar el reproductor de vídeo.");
  }
}
...
function setStatus(message, isSuccess = false, isWarning = false) {
  STATUS_DISPLAY.textContent = `Estado: ${message}`;
  if (isSuccess) {
...
function checkDisplayMediaSupport() {
  if (navigator.mediaDevices && navigator.mediaDevices.getDisplayMedia) {
    ...
    setError(
      "Tu navegador no admite la compartición de pantalla. No puedes iniciar una transmisión."
    );
    if (streamTabButton) {
      streamTabButton.disabled = true;
      streamTabButton.title =
        "Tu navegador no admite la compartición de pantalla."; // Add tooltip
    }
    ...
    if (initiateStreamButton) {
      ...
      initiateStreamButton.title =
        "Tu navegador no admite la compartición de pantalla.";
    }
  }
}
...
function connectWebSocket(url) {
  return new Promise((resolve, reject) => {
    setStatus(`Conectando con el servidor de señalización...`);
    ...
    ws.onerror = (event) => {
      ...
      if (!wsConnected) {
        ...
        setError(`Fallo en la conexión de señalización: ${errorMsg}`);
        ...
        setError(`Error de señalización: ${errorMsg}`);
        setStatus(
          "Aviso: Señalización interrumpida. Las conexiones entre pares podrían seguir activas.",
          false,
          true
        );
    ...
    ws.onclose = (event) => {
      ...
      if (role === "viewer" && code === 1000 && reason === "NO_STREAMER") {
        ...
        setError(`El emisor '${targetCode}' no existe o ya no está activo.`);
      ...
      if (!handledSpecific && wsConnected && code !== 1000 && code !== 1001) {
        setStatus(
          `Aviso: Señalización desconectada inesperadamente (Código: ${code}). Las conexiones entre pares podrían seguir activas.`,
          false,
          true
        );
...
async function startStreamerInitialization() {
  ...
  if (!isDisplayMediaSupported) {
    ...
    setError(
      "No se puede iniciar la transmisión: tu navegador no admite la compartición de pantalla."
    );
  ...
  try {
    ...
    setStatus("Solicitando captura de pantalla...");
    ...
    if (!localStream || !localStream.active) {
      throw new Error("La captura de pantalla falló o se denegó el permiso.");
    }
    ...
    setStatus("Configuración de la transmisión completa. Esperando código...", true); // Status indicates ready for code
  } catch (error) {
    setError(`No se pudo iniciar la transmisión: ${error.message}`);
...
async function startLocalStream() {
  ...
  if (!isDisplayMediaSupported) {
    ...
    throw new Error(
      "La compartición de pantalla (getDisplayMedia) no es compatible con este navegador."
    );
  }
  // --- End NEW ---
  ...
  try {
    ...
    if (
      ...
      throw new Error("La captura de pantalla devolvió un flujo inválido o vacío.");
    }
    ...
    if (audioTracks.length > 0) {
      ...
      setStatus(
        "Compartición de pantalla iniciada, pero la captura de audio falló o no se permitió.",
        false,
        true
      );
    }
    ...
    if (
      ...
      throw new Error("Se denegó el permiso para compartir la pantalla.");
    } else if (
      error.name === "NotFoundError" ||
      error.name === "DevicesNotFoundError"
    ) {
      throw new Error("No se encontraron dispositivos de captura de pantalla compatibles.");
    } else if (
      error.name === "NotReadableError" ||
      error.name === "TrackStartError"
    ) {
      throw new Error(
        "La fuente de pantalla o audio está ocupada o no se puede leer."
      );
    } else if (error.name === "AbortError") {
      throw new Error("Se canceló la solicitud de compartición de pantalla.");
    } else if (error.name === "OverconstrainedError") {
      console.error("OverconstrainedError Details:", error.constraint);
      throw new Error(
        `El dispositivo/navegador no puede cumplir las restricciones de audio/vídeo solicitadas (Restricción: ${
          error.constraint || "desconocida"
        }). Intenta simplificar las restricciones.`
      );
    } else {
...
async function connectViewer() {
  ...
  if (!viewerTargetCode || viewerTargetCode.length !== 5) {
    setError("Formato de código no válido. Introduce el código de 5 caracteres.");
    return;
  }

  clearError(); // Clear previous errors
  setStatus(`Conectando con el código ${viewerTargetCode}...`);
  ...
  try {
    ...
    signalingTimeout = setTimeout(() => {
      setError(
        `Tiempo de espera agotado esperando al emisor: ${viewerTargetCode}.`
      );
    ...
    if (!pc) {
      throw new Error("No se pudo crear la RTCPeerConnection.");
    }
    ...
    if (error.message && error.message.includes("404")) {
      ...
      setError(
        `Conexión fallida: el código de emisor '${viewerTargetCode}' no existe o está inactivo.`
      );
    } else {
      setError(`No se pudo conectar como espectador: ${error.message}`);
    }
  ...
  try {
    ...
    if (role === "streamer") {
      ...
      if (message.code) {
        ...
        setStatus("Transmitiendo - Comparte este código con los espectadores.", true);
      ...
      if (message.from && message.data && message.data.type === "offer") {
        ...
        if (!localStream || !localStream.active) {
          setError(
            `Emisor: el flujo de pantalla no está activo. No se puede responder a la oferta de ${viewerId}.`
          );
          console.error(
        ...
        if (activeTracks.length === 0) {
          setError(
            `Emisor: no hay pistas activas en el flujo de pantalla. No se puede responder a la oferta de ${viewerId}.`
          );
          console.error(
      ...
      if (message.type === "answer" && message.sdp) {
        ...
        if (!pc.currentRemoteDescription) {
          try {
            ...
            setError(
              `Espectador: no se pudo procesar la respuesta del emisor: ${e.message}`
            );
            closeConnections();
          }
        } else {
    ...
    setError(`Error al procesar el mensaje de señalización: ${error.message}`);
...
function sendMessage(payload, targetViewerId = null) {
  ...
  if (!ws || ws.readyState !== WebSocket.OPEN) {
    ...
    if (
      ...
      if (!STATUS_DISPLAY.className.includes("warning")) {
        setError("No se puede enviar el mensaje: la conexión de señalización está inactiva.");
      }
    } else {
      console.log(
  ...
  try {
    ...
    setError(`No se pudo enviar el mensaje de señalización: ${e.message}`);
    console.error(
...
async function createPeerConnection(viewerId = null) {
  ...
  try {
    ...
    setError(
      `Fallo en la configuración de WebRTC: no se pudo crear la PeerConnection. ${e.message}`
    );
  ...
  pc.oniceconnectionstatechange = () => {
    ...
    switch (state) {
      ...
      case "failed":
        ...
        setError(
          `Fallo en la conexión WebRTC (negociación ICE) ${
            role === "streamer" ? "para el espectador " + viewerId : ""
          }.`
        );
    ...
    switch (state) {
      ...
      case "closed":
        ...
        if (!ERROR_DISPLAY.textContent) {
          setStatus(
            role === "viewer"
              ? "La transmisión terminó o la conexión se cerró."
              : `Conexión con el espectador ${viewerId || ""} cerrada.`
          );
        }
...
function closeConnections(closeWsExplicitly = true) {
  ...
  setStatus("Cerrando conexiones..."); // Initial status update for cleanup process
  ...
  if (!ERROR_DISPLAY.textContent.includes("not supported by your browser")) {
    setStatus("Conexiones cerradas. Listo."); // Indicate ready state
  }
...
setStatus("Inactivo"); // Set initial status after checks
