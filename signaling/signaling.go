...
func (s *Signaler) HandleStreamerWS(rw http.ResponseWriter, req *http.Request) {
	...
	if err != nil {
		slog.Error("No se pudo aceptar la conexión WS", "type", "streamer", "error", err)
		return
	}
	...
	context.AfterFunc(ctx, func() {
		...
		if errors.As(err, &closeError) {
			slog.Info("Emisor desconectado", "code", code)
			return
		}

		slog.Error("Contexto del emisor cerrado", "error", err)
	})
	...
	slog.Info("Emisor conectado", "code", code)
	s.SR.Mu.Unlock()
}

const clientSignalingTimeout = time.Minute

var errClientSignalTimeout = errors.New("el cliente ha superado su límite de tiempo de señalización")
...
func (s *Signaler) HandleViewerWS(rw http.ResponseWriter, req *http.Request) {
	...
	conn, err := websocket.Accept(rw, req, nil)
	if err != nil {
		slog.Error("No se pudo aceptar la conexión WS", "type", "viewer", "error", err)
		return
	}
	...
	context.AfterFunc(ctx, func() {
		...
		var closeError websocket.CloseError
		if errors.As(err, &closeError) {
			slog.Info("Conexión WS del espectador liberada", "id", viewerID)
			return
		}

		if errors.Is(err, errClientSignalTimeout) {
			slog.Info("Tiempo de espera agotado en el WS del espectador", "id", viewerID)
			return
		}

		slog.Error("Contexto del espectador cerrado", "error", err)
	})
	...
	slog.Info("Espectador conectado", "id", viewerID, "code", code)
	s.VR.Mu.Unlock()
}
...
func (s *Streamer) handleOut(vr *ViewerRegistry) {
	...
	for {
		...
		case out := <-s.Out:
			...
			if err != nil {
				logger.Info("UUID de destino no válido", "id", out.To)
				continue
			}
			...
			if !ok {
				logger.Info("El UUID de destino no existe", "id", viewerID)
				continue
			}
...
func (s *Streamer) handleIn() {
	...
	for {
		...
		case in := <-s.In:
			...
			if err == nil {
				logger.Debug("Emisor recibió mensaje", "id", in.From)
				continue
			}
			s.Cancel(err)
		}
...
func (v *Viewer) readWS() {
	...
	for {
		...
		logger.Debug("Espectador envió mensaje")
	}
}
